...
//need an if statment where if negative then turn array in to decimals
...
const extractStats = (arr) => arr.map( element => {
  // Solution code here...arr.map( element => {
  let result = {};
  result.name = element.stat.name;
  result.total = element.effort + element.baseStat;
  return result;
});
